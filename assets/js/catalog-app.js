...
(() => {
  ...
  function search() {
    ...
    const row=({file:f})=>`<tr class="file-row ${state.selected.has(f.id)?'selected':''} ${state.mode==='preview'&&state.preview===f.id?'previewing':''}" data-file="${f.id}">
      <td class="compare-cell"><input type="checkbox" data-select="${f.id}" ${state.selected.has(f.id)?'checked':''} aria-label="比较 ${esc(f.label)}"></td>
    ...
    $('results').innerHTML=state.results.length?`<table class="file-table" aria-label="气体文件检索结果"><thead><tr><th scope="col">比较</th><th scope="col">气体配方</th><th scope="col">温度 (°C)</th><th scope="col">压强 (atm)</th><th scope="col">操作</th></tr></thead><tbody>${state.results.map(row).join('')}</tbody></table>`:'';
  }
