...
class CatalogueTests(unittest.TestCase):
    ...
    def test_disjoint_magnetic_tables_and_overlap_rejection(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            def with_angle(text, value):
                return re.sub(r'(E-B angles\s+).*?(?=B fields)',
                              lambda m: m[1] + str(value) + '\n ', text, flags=re.S)
            (folder / 'a.gas').write_text(with_angle(with_b(0), 0))
            (folder / 'b.gas').write_text(with_angle(with_b(0), 1.5707963))
            _, report = catalog.assemble(root)
            self.assertEqual(len(report['rejected']), 1)
            self.assertIn('B=0', report['rejected'][0]['reason'])
            (folder / 'a.gas').write_text(with_angle(with_b(100), 0))
            (folder / 'b.gas').write_text(with_angle(with_b(100), 1.5707963))
            data, report = catalog.assemble(root)
            self.assertEqual(len(data['files']), 2)
            self.assertFalse(report['rejected'])
            (folder / 'a.gas').write_text(with_b(0))
